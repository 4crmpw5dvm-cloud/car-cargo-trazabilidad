...
el.print.onclick=()=>{show('reporte');let old=$('printEvidence');if(old)old.remove();let x=document.createElement('div');x.id='printEvidence';x.innerHTML=st.clients.filter(c=>c.delivery&&(c.delivery.photo||c.delivery.sig)).map(c=>`<div style="break-inside:avoid;margin-top:24px"><h3>Evidencias - ${esc(c.name)}</h3>${c.delivery.photo?`<p><b>Foto</b></p><img src="${c.delivery.photo}" style="max-width:320px;max-height:240px;object-fit:contain">`:''}${c.delivery.sig?`<p><b>Firma</b></p><img src="${c.delivery.sig}" style="max-width:320px;max-height:160px;object-fit:contain">`:''}</div>`).join('');document.getElementById('reporte').appendChild(x);setTimeout(()=>window.print(),500)};
